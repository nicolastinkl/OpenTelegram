...
public class GiftPremiumBottomSheet extends BottomSheetWithRecyclerListView {
    ...
    public GiftPremiumBottomSheet(BaseFragment baseFragment, TLRPC$User tLRPC$User) {
        ...
        if (userFull != null) {
            ...
            if (BuildVars.useInvoiceBilling()) {
                ...
                while (it2.hasNext()) {
                    it2.next().setPricePerMonthRegular(j);
                }
    ...
    public /* synthetic */ void lambda$new$1(final long j, BillingResult billingResult, List list) {
        ...
        Iterator<GiftTier> it3 = this.giftTiers.iterator();
        while (it3.hasNext()) {
            it3.next().setPricePerMonthRegular(j2);
        }
    ...
    private void onGiftPremium() {
        ...
        tLRPC$TL_inputStorePaymentGiftPremium.currency = giftTier.googlePlayProductDetails.getOneTimePurchaseOfferDetails().getPriceCurrencyCode();
        tLRPC$TL_inputStorePaymentGiftPremium.amount = (long) ((r2.getPriceAmountMicros() / Math.pow(10.0d, 6.0d)) * Math.pow(10.0d, BillingController.getInstance().getCurrencyExp(tLRPC$TL_inputStorePaymentGiftPremium.currency)));
