        ...
        if (this.chartData != 0) {
            for (int i3 = 0; i3 < size; i3++) {
                ...
                if (barViewData.enabled || barViewData.alpha != 0.0f) {
                    ...
                    while (i4 < length) {
                        if (iArr[i4] >= 0) {
                            float f4 = this.chartData.xPercentage[i4] * this.pickerWidth;
                            float f5 = (f - ((iArr[i4] / (BaseChartView.ANIMATE_PICKER_SIZES ? this.pickerMaxHeight : r14.maxValue)) * f3)) * (i2 - measuredHeight2);
