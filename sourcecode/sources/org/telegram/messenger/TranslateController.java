...
public class TranslateController extends BaseController {
    ...
    public static ArrayList<Language> getLanguages() {
        ...
        for (int i = 0; i < allLanguages.size(); i++) {
            ...
            if (language.displayName != null) {
                ...
                sb.append(str2);
                language.f29q = sb.toString().toLowerCase();
        ...
        while (it.hasNext()) {
            ...
            if (!TextUtils.equals(next, str) && (!"no".equals(str) || !"nb".equals(next))) {
                if (!"nb".equals(str) || !"no".equals(next)) {
                    ...
                    language.ownDisplayName = TranslateAlert2.capitalFirst(TranslateAlert2.systemLanguageName(language.code, true));
                    if (language.displayName != null) {
                        language.f29q = (language.displayName + " " + language.ownDisplayName).toLowerCase();
                        arrayList.add(language);
                    }
                }
            }
    ...
    private void checkDialogTranslatable(MessageObject messageObject) {
        ...
        boolean z = isTranslatable(messageObject) && ((str2 = messageObject.messageOwner.originalLanguage) == null || UNKNOWN_LANGUAGE.equals(str2));
        boolean z2 = (!isTranslatable(messageObject) || (str = messageObject.messageOwner.originalLanguage) == null || UNKNOWN_LANGUAGE.equals(str) || RestrictedLanguagesSelectActivity.getRestrictedLanguages().contains(messageObject.messageOwner.originalLanguage)) ? false : true;
        if (z) {
        ...
        if (size3 < 8 || size / (size + r2) < REQUIRED_PERCENTAGE_MESSAGES_TRANSLATABLE || size2 / size3 >= REQUIRED_MIN_PERCENTAGE_MESSAGES_UNKNOWN) {
            return;
        }
