...
class ClockHandView extends View {
    ...
    private void drawSelector(Canvas canvas) {
        ...
        float cos2 = (this.circleRadius * ((float) Math.cos(this.degRad))) + width;
        float f = height;
        float sin = (this.circleRadius * ((float) Math.sin(this.degRad))) + f;
        this.paint.setStrokeWidth(0.0f);
        canvas.drawCircle(cos2, sin, this.selectorRadius, this.paint);
        double sin2 = Math.sin(this.degRad);
        double cos3 = Math.cos(this.degRad);
        this.paint.setStrokeWidth(this.selectorStrokeWidth);
        canvas.drawLine(width, f, r1 + ((int) (cos3 * r6)), height + ((int) (r6 * sin2)), this.paint);
